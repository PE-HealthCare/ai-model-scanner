...
from src.p1_static_engine.analyzer import TrustedModelContext, build_mock_features, extract_features, intake_model
...
def run_real_pipeline(model_path: str | Path, declared_architecture: str) -> Path:
    """Execute P1 real intake + feature extraction, producing features.json."""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) >= 4 and sys.argv[2] == "--arch":
        out_path = run_real_pipeline(sys.argv[1], sys.argv[3])
        print(f"features: {out_path}")
        print("Phase 1 real pipeline: PASS (contract validation succeeded)")
    else:
