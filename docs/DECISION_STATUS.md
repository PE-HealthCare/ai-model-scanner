...
| D5 | **RESOLVED / LOCKED — METHODOLOGY BOUNDARY; EXACT OPERATOR PENDING** | D5 owns reduction of valid per-layer static/tampering evidence to model-level evidence consumed by the frozen MRS formulas. Parts 1–3 lock evidence lineage, `S_static` semantics, and localized/distributed/mixed threat-model coverage. **Part 4 locks candidate comparison/elimination without selecting a final operator. Part 5 locks the authoritative D7-governed per-layer × per-feature evidence representation, preserving layer identity, feature identity, validity/degeneracy state, and provenance. Part 6 now locks the numerical candidate-evaluation methodology, including complete reduction-path testing, hierarchical feature/layer reduction checks, localized/distributed/mixed scenarios, layer/feature-count invariance, D7 blocked/invalid handling, monotonicity, boundedness, dilution resistance, determinism, and layer-order invariance.** Part 6 does not select the final operator, authorize a production per-layer scalar, or authorize a final Z→[0,1] mapping. The exact aggregation operator and evidence-backed parameters remain pending Part 7. See `docs/D5_PART1_PER_LAYER_EVIDENCE_DECISION.md`, `docs/D5_PART2_MODEL_LEVEL_S_STATIC_SEMANTICS_DECISION.md`, `docs/D5_PART3_THREAT_MODEL_DECISION.md`, `docs/D5_PART4_CANDIDATE_COMPARISON_DECISION.md`, `docs/D5_PART5_AUTHORITATIVE_PER_LAYER_EVIDENCE_DECISION.md`, and `docs/D5_PART6_NUMERICAL_CANDIDATE_EVALUATION_DECISION.md`. |
