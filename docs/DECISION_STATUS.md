...
| D6 | **RESOLVED / LOCKED — PARTS 1 & 2** | P1 owns authoritative production/preservation of per-layer evidence and authoritative layer identity; P3/D6 owns final highest-risk-layer selection and security-report presentation. D6 selects `argmax E(l)` over eligible D5 per-layer evidence, where `E(l)=max_f(|Z(l,f)|/(1+|Z(l,f)|))` using only D7-valid finite evidence. Invalid/missing evidence is not imputed or fabricated; `DEGENERATE_DEVIATION` and unavailable baseline remain non-eligible states. No eligible layer reports `highest_risk_layer = unavailable`; one eligible layer is selected; exact ties use canonical authoritative `layer_id` ordering. Quantized valid static evidence follows the same rule without invented behavioral evidence. TreeSHAP remains separate and is not used for layer selection. D6 does not recalculate upstream signals, redefine `S_static`, calculate MRS, modify thresholds, or invent layer granularity. See `docs/D6_HIGHEST_RISK_LAYER_DECISION.md`. |
...
This registry does not authorize silent implementation of D4 or completion claims for unresolved D9 evidence. **D5 Part 7 and D6 Parts 1 & 2 are now locked; production implementation must reproduce their exact approved boundaries.** The frozen architecture remains authoritative.
