...
| D5 | **RESOLVED / LOCKED — EXACT OPERATOR** | D5 owns reduction of valid per-layer static/tampering evidence to model-level evidence consumed by the frozen MRS formulas. Parts 1–3 lock evidence lineage, `S_static` semantics, and localized/distributed/mixed threat-model coverage. Part 4 locks candidate comparison/elimination. Part 5 locks the authoritative D7-governed per-layer × per-feature evidence representation, preserving layer identity, feature identity, validity/degeneracy state, and provenance. Part 6 locks the numerical candidate-evaluation methodology. **Part 7 now locks the exact construction:** `e(l,f)=|Z(l,f)|/(1+|Z(l,f)|)` for D7-valid finite evidence; `E(l)=max_f e(l,f)`; `S_static=1-product_l(1-E(l))`. Invalid/blocked/degenerate evidence is not silently imputed or converted to zero. No Gaussian assumption, hidden calibration, new threshold, or MRS change is authorized. See `docs/D5_PART1_PER_LAYER_EVIDENCE_DECISION.md`, `docs/D5_PART2_MODEL_LEVEL_S_STATIC_SEMANTICS_DECISION.md`, `docs/D5_PART3_THREAT_MODEL_DECISION.md`, `docs/D5_PART4_CANDIDATE_COMPARISON_DECISION.md`, `docs/D5_PART5_AUTHORITATIVE_PER_LAYER_EVIDENCE_DECISION.md`, `docs/D5_PART6_NUMERICAL_CANDIDATE_EVALUATION_DECISION.md`, and `docs/D5_PART7_EXACT_OPERATOR_DECISION.md`. |
| D6 | **PART 1 RESOLVED / LOCKED — AUTHORITY & EVIDENCE CONTRACT; PART 2 REQUIRED** | P1 owns authoritative production/preservation of per-layer evidence and authoritative layer identity; P3/D6 owns final highest-risk-layer selection and security-report presentation. D6 consumes preserved upstream per-layer evidence with validity/provenance context and does not recalculate upstream signals, rewrite D7 states, invent layer granularity, invent quantized behavioral evidence, or use TreeSHAP for layer selection. Exact selection operation and edge-case semantics remain Part 2 decisions. See `docs/D6_HIGHEST_RISK_LAYER_DECISION.md`. |
...
This registry does not authorize silent implementation of D4 or D6 Part 2 or completion claims for unresolved D9 evidence. D5 Part 7 and D6 Part 1 are now locked; production implementation must reproduce their exact approved boundaries. The frozen architecture remains authoritative.
