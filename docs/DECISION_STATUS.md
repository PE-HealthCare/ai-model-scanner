...
| D5 | **RESOLVED / LOCKED — METHODOLOGY BOUNDARY; EXACT OPERATOR PENDING** | D5 owns reduction of valid per-layer static/tampering evidence to model-level evidence consumed by the frozen MRS formulas. Part 1 locks authoritative per-layer evidence lineage and preservation of layer identity/validity provenance. Part 2 locks the semantic target and evaluation requirements for model-level `S_static`. Part 3 locks threat-model coverage: localized, distributed, and mixed static manipulation must all be considered. **Part 4 locks candidate comparison/elimination without selecting a final operator. Part 5 now locks the authoritative D7-governed per-layer × per-feature evidence representation, preserving layer identity, feature identity, validity/degeneracy state, and provenance. It does not introduce a new per-layer scalar or normalize Z into [0,1].** The exact aggregation operator and evidence-backed parameters remain pending explicit authorization; no operator may be inferred from implementation convenience. See `docs/D5_PART1_PER_LAYER_EVIDENCE_DECISION.md`, `docs/D5_PART2_MODEL_LEVEL_S_STATIC_SEMANTICS_DECISION.md`, `docs/D5_PART3_THREAT_MODEL_DECISION.md`, `docs/D5_PART4_CANDIDATE_COMPARISON_DECISION.md`, and `docs/D5_PART5_AUTHORITATIVE_PER_LAYER_EVIDENCE_DECISION.md`. |
