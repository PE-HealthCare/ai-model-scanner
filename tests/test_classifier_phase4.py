...
class TestPhase4Classifier(unittest.TestCase):
    def _model(self, path: Path):
        ...
        X=np.vstack([rng.normal(size=(30,10)),rng.normal(loc=1,size=(30,10))]); y=np.r_[np.zeros(30),np.ones(30)]
    ...
    def test_real_classifier_and_tree_shap_contract(self):
        ...
        self.assertEqual(result["producer"],"P3"); self.assertEqual(result["mock_status"],"VERIFIED-REAL"); self.assertTrue(0<=result["p_tamper"]<=1); self.assertEqual(list(result["shap_attributions"]),list(FEATURE_NAMES))
